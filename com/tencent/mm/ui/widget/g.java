...
public final class g {
    ...
    public static void a(a aVar) {
        x.d("MicroMsg.SwipeBackHelper", "pushCallback size %d, %s", Integer.valueOf(uHV.size()), aVar);
        uHV.add(0, new WeakReference(aVar));
    }

    public static boolean b(a aVar) {
        x.d("MicroMsg.SwipeBackHelper", "popCallback size %d, %s", Integer.valueOf(uHV.size()), aVar);
        if (aVar == null) {
            return true;
        }
        LinkedList linkedList = new LinkedList();
        for (int i = 0; i < uHV.size(); i++) {
            if (aVar == ((WeakReference) uHV.get(i)).get()) {
                uHV.remove(i);
        ...
        if (aVar.forceRemoveNoMatchOnPath() || linkedList.size() != r4) {
            ...
            while (it.hasNext()) {
                WeakReference weakReference = (WeakReference) uHV.remove(((Integer) it.next()).intValue());
    ...
    public static void aD(float f) {
        ...
        a aVar = (a) ((WeakReference) uHV.get(0)).get();
        if (aVar == null) {
