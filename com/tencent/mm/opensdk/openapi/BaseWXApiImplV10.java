...
class BaseWXApiImplV10 implements IWXAPI {
    ...
    private boolean handleWxInternalRespType(String str, IWXAPIEventHandler iWXAPIEventHandler) {
        ...
        try {
            ...
            if (d.a(queryParameter)) {
                ...
                wXNontaxPay$Resp.wxOrderId = parse.getQueryParameter("wx_order_id");
                iWXAPIEventHandler.onResp(wXNontaxPay$Resp);
                return true;
                ...
                if (queryParameter2 != null && queryParameter2.length() > 0) {
                    resp4.errCode = d.b(queryParameter2);
                }
                resp4.openId = parse.getQueryParameter("openid");
                resp4.unionId = parse.getQueryParameter("unionid");
                resp4.nickname = parse.getQueryParameter("nickname");
                resp4.errStr = parse.getQueryParameter("errmsg");
    ...
    public boolean isWXAppSupportAPI() {
        if (!this.detached) {
            return getWXAppSupportAPI() >= 620823808;
        } else {
