...
public final class aa {
            ...
            if (Modifier.isStatic(field.getModifiers()) && ((field.getName().startsWith("USERINFO_") || field.getName().startsWith("NEW_BANDAGE_") || field.getName().startsWith("DYNAMIC_CONFIG_")) && field.getType().toString().equals("int"))) {
                try {
                    ...
                    if (sparseArray.get(i, null) != null) {
                        x.e("MicroMsg.ConstantsStorage", "%s and %s has same value(0x%05X)!!!", sparseArray.get(i), field.getName(), Integer.valueOf(i));
                        return false;
                    }
                    sparseArray.put(i, field.getName());
                } catch (Throwable e) {
                    x.e("MicroMsg.ConstantsStorage", "exception:%s", bi.i(e));
                } catch (Throwable e2) {
                    x.e("MicroMsg.ConstantsStorage", "exception:%s", bi.i(e2));
                }
            }
        }
        if (sparseArray.size() > 511) {
            x.e("MicroMsg.ConstantsStorage", "constants values size(%d) is over the limit(%d)!!!", Integer.valueOf(sparseArray.size()), Integer.valueOf(511));
            return false;
        }
        x.i("MicroMsg.ConstantsStorage", "checkDuplicateUserInfo values size: %d", Integer.valueOf(sparseArray.size()));
        return ckU();
    }
    ...
    private static final boolean ckU() {
        ...
        for (a aVar : a.values()) {
            ...
            String XN = XN(aVar.name());
            if (bi.oW(XN)) {
                return false;
            }
            if (arrayList.contains(XN)) {
                x.e("MicroMsg.ConstantsStorage", "BusinessInfoKey check error: redefinition of [%s] which already defined!", XN);
                return false;
            }
            arrayList.add(XN);
        }
    ...
    private static final String XN(String str) {
        ...
        if (split == null || split.length < 4) {
            String str3 = "MicroMsg.ConstantsStorage";
            str2 = "BusinessInfoKey parse failed: key:%s split by '_'  fileds len too short : %d , at least 4";
        ...
        while (i2 < split.length) {
            if (split[i2] == null || split[i2].length() <= 0) {
                x.e("MicroMsg.ConstantsStorage", "BusinessInfoKey parse failed: name:%s , [%s] too short ( <1 ) ", str, split[i2]);
                return null;
            }
            i2++;
        }
        ...
        if (i2 < 3) {
            String str4 = "MicroMsg.ConstantsStorage";
            String str5 = "BusinessInfoKey parse failed: name:%s split by '_'  fileds len too short : %d , at least 3";
        ...
        x.e("MicroMsg.ConstantsStorage", "BusinessInfoKey parse failed: name[%s], invalid type:%s ", str, str2);
        return null;
    }
}
