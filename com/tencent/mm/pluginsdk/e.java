...
public final class e {
    public static int eK(Context context) {
        ...
        if (i <= 0) {
            return context.getResources().getDimensionPixelSize(com.tencent.mm.w.a.e.DefaultActionbarHeightPort);
        }
        return i;
    }
    ...
    public static void k(MMActivity mMActivity) {
        if (VERSION.SDK_INT >= 16) {
    ...
    public static int cM(Context context) {
        ...
        if (displayMetrics.widthPixels > displayMetrics.heightPixels) {
            return context.getResources().getDimensionPixelSize(com.tencent.mm.w.a.e.DefaultActionbarHeightLand);
        }
        return context.getResources().getDimensionPixelSize(com.tencent.mm.w.a.e.DefaultActionbarHeightPort);
    }

    public static int eL(Context context) {
        int ap = s.ap(context, -1);
        Rect rect;
        if (ap > 0) {
            rect = new Rect();
            ((Activity) context).getWindow().getDecorView().getWindowVisibleDisplayFrame(rect);
            return rect.top > ap ? 0 : ap;
        } else if (!(context instanceof Activity)) {
            return a.fromDPToPix(context, 20);
        } else {
            rect = new Rect();
            ((Activity) context).getWindow().getDecorView().getWindowVisibleDisplayFrame(rect);
            ap = ((Activity) context).getWindow().getDecorView().getHeight();
            int[] iArr = new int[2];
            ((Activity) context).getWindow().getDecorView().getLocationOnScreen(iArr);
            if (ap - rect.height() < 0 || iArr[1] <= 200) {
                return rect.top;
            }
            return ap - rect.height();
        }
    }
}
