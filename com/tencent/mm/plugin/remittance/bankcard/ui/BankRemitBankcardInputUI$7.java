package com.tencent.mm.plugin.remittance.bankcard.ui;

import com.tencent.mm.ab.l;
import com.tencent.mm.sdk.platformtools.x;
import com.tencent.mm.wallet_core.c.h.a;

class BankRemitBankcardInputUI$7 implements a {
    final /* synthetic */ BankRemitBankcardInputUI mve;

    BankRemitBankcardInputUI$7(BankRemitBankcardInputUI bankRemitBankcardInputUI) {
        this.mve = bankRemitBankcardInputUI;
    }

    public final void g(int i, int i2, String str, l lVar) {
        x.e("MicroMsg.BankRemitBankcardInputUI", "net error: %s", new Object[]{lVar});
        BankRemitBankcardInputUI.b(this.mve, true);
    }
}
