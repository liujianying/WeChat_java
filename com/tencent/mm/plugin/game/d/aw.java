package com.tencent.mm.plugin.game.d;

import com.tencent.mm.protocal.c.bhd;
import com.tencent.mm.protocal.c.fk;
import f.a.a.b;
import f.a.a.c.a;
import java.util.LinkedList;

public final class aw extends bhd {
    public String eJQ;
    public String jRj;
    public int jRk;
    public d jRl;

    protected final int a(int i, Object... objArr) {
        int fS;
        if (i == 0) {
            a aVar = (a) objArr[0];
            if (this.jRj == null) {
                throw new b("Not all required fields were included: Lang");
            }
            if (this.shX != null) {
                aVar.fV(1, this.shX.boi());
                this.shX.a(aVar);
            }
            if (this.jRj != null) {
                aVar.g(2, this.jRj);
            }
            if (this.eJQ != null) {
                aVar.g(3, this.eJQ);
            }
            aVar.fT(4, this.jRk);
            if (this.jRl == null) {
                return 0;
            }
            aVar.fV(5, this.jRl.boi());
            this.jRl.a(aVar);
            return 0;
        } else if (i == 1) {
            if (this.shX != null) {
                fS = f.a.a.a.fS(1, this.shX.boi()) + 0;
            } else {
                fS = 0;
            }
            if (this.jRj != null) {
                fS += f.a.a.b.b.a.h(2, this.jRj);
            }
            if (this.eJQ != null) {
                fS += f.a.a.b.b.a.h(3, this.eJQ);
            }
            fS += f.a.a.a.fQ(4, this.jRk);
            if (this.jRl != null) {
                fS += f.a.a.a.fS(5, this.jRl.boi());
            }
            return fS;
        } else if (i == 2) {
            f.a.a.a.a aVar2 = new f.a.a.a.a((byte[]) objArr[0], unknownTagHandler);
            for (fS = bhd.a(aVar2); fS > 0; fS = bhd.a(aVar2)) {
                if (!super.a(aVar2, this, fS)) {
                    aVar2.cJS();
                }
            }
            if (this.jRj != null) {
                return 0;
            }
            throw new b("Not all required fields were included: Lang");
        } else if (i != 3) {
            return -1;
        } else {
            f.a.a.a.a aVar3 = (f.a.a.a.a) objArr[0];
            aw awVar = (aw) objArr[1];
            int intValue = ((Integer) objArr[2]).intValue();
            LinkedList IC;
            int size;
            byte[] bArr;
            f.a.a.a.a aVar4;
            boolean z;
            switch (intValue) {
                case 1:
                    IC = aVar3.IC(intValue);
                    size = IC.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) IC.get(intValue);
                        fk fkVar = new fk();
                        aVar4 = new f.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = fkVar.a(aVar4, fkVar, bhd.a(aVar4))) {
                        }
                        awVar.shX = fkVar;
                    }
                    return 0;
                case 2:
                    awVar.jRj = aVar3.vHC.readString();
                    return 0;
                case 3:
                    awVar.eJQ = aVar3.vHC.readString();
                    return 0;
                case 4:
                    awVar.jRk = aVar3.vHC.rY();
                    return 0;
                case 5:
                    IC = aVar3.IC(intValue);
                    size = IC.size();
                    for (intValue = 0; intValue < size; intValue++) {
                        bArr = (byte[]) IC.get(intValue);
                        d dVar = new d();
                        aVar4 = new f.a.a.a.a(bArr, unknownTagHandler);
                        for (z = true; z; z = dVar.a(aVar4, dVar, bhd.a(aVar4))) {
                        }
                        awVar.jRl = dVar;
                    }
                    return 0;
                default:
                    return -1;
            }
        }
    }
}
