package com.tencent.mm.protocal.c;

import com.tencent.mm.bk.a;
import f.a.a.b;
import java.util.LinkedList;

public final class bsw extends a {
    public String aAL;
    public String mxA;
    public LinkedList<String> sqo = new LinkedList();
    public String value;

    protected final int a(int i, Object... objArr) {
        int h;
        if (i == 0) {
            f.a.a.c.a aVar = (f.a.a.c.a) objArr[0];
            if (this.aAL == null) {
                throw new b("Not all required fields were included: key");
            } else if (this.value == null) {
                throw new b("Not all required fields were included: value");
            } else {
                if (this.aAL != null) {
                    aVar.g(1, this.aAL);
                }
                if (this.value != null) {
                    aVar.g(2, this.value);
                }
                if (this.mxA != null) {
                    aVar.g(3, this.mxA);
                }
                aVar.d(4, 1, this.sqo);
                return 0;
            }
        } else if (i == 1) {
            if (this.aAL != null) {
                h = f.a.a.b.b.a.h(1, this.aAL) + 0;
            } else {
                h = 0;
            }
            if (this.value != null) {
                h += f.a.a.b.b.a.h(2, this.value);
            }
            if (this.mxA != null) {
                h += f.a.a.b.b.a.h(3, this.mxA);
            }
            return h + f.a.a.a.c(4, 1, this.sqo);
        } else if (i == 2) {
            byte[] bArr = (byte[]) objArr[0];
            this.sqo.clear();
            f.a.a.a.a aVar2 = new f.a.a.a.a(bArr, unknownTagHandler);
            for (h = a.a(aVar2); h > 0; h = a.a(aVar2)) {
                if (!super.a(aVar2, this, h)) {
                    aVar2.cJS();
                }
            }
            if (this.aAL == null) {
                throw new b("Not all required fields were included: key");
            } else if (this.value != null) {
                return 0;
            } else {
                throw new b("Not all required fields were included: value");
            }
        } else if (i != 3) {
            return -1;
        } else {
            f.a.a.a.a aVar3 = (f.a.a.a.a) objArr[0];
            bsw bsw = (bsw) objArr[1];
            switch (((Integer) objArr[2]).intValue()) {
                case 1:
                    bsw.aAL = aVar3.vHC.readString();
                    return 0;
                case 2:
                    bsw.value = aVar3.vHC.readString();
                    return 0;
                case 3:
                    bsw.mxA = aVar3.vHC.readString();
                    return 0;
                case 4:
                    bsw.sqo.add(aVar3.vHC.readString());
                    return 0;
                default:
                    return -1;
            }
        }
    }
}
