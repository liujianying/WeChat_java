...
public class MP3FileCheck {
    ...
    private boolean syncFrame() {
        ...
        while (!this.eof) {
            ...
            while (!available(byte2int, this.headerMask)) {
                ...
                i2 = this.pos;
                this.pos = i2 + 1;
                byte2int |= bArr[i2] & WebView.NORMAL_MODE_ALPHA;
    ...
    private int byte2int(byte[] bArr, int i) {
        return (((bArr[i] << 24) | ((bArr[i + 1] & WebView.NORMAL_MODE_ALPHA) << 16)) | ((bArr[i + 2] & WebView.NORMAL_MODE_ALPHA) << 8)) | (bArr[i + 3] & WebView.NORMAL_MODE_ALPHA);
    }
