...
public class WakerLock {
    ...
    public WakerLock(Context context, String str) {
        ...
        this.mHandler = new ag(context.getMainLooper());
    ...
    private static final boolean checkShouldLock() {
        if (bi.bH(lastChecktime) > 10000 || shouldLock == null) {
            boolean z = (l.fs(ad.getContext()) || l.ft(ad.getContext())) ? false : true;
            shouldLock = Boolean.valueOf(z);
            x.i(TAG, "checkShouldLock screen:%b chatging:%b res:%b checkTime:%d ", Boolean.valueOf(r3), Boolean.valueOf(r4), shouldLock, Long.valueOf(bi.bH(lastChecktime)));
            lastChecktime = bi.VF();
        }
