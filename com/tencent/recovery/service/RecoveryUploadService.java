...
public class RecoveryUploadService extends IntentService {
    ...
    public static synchronized void startAlarm(Context context, String str) {
        synchronized (RecoveryUploadService.class) {
            if (!Util.oW(str)) {
                CommonOptions hj = OptionFactory.hj(context);
                ...
                RecoveryLog.i(TAG, "startAlarm %s lastUploadTime %s", str, Util.gS(j));
                ...
                if (service == null || currentTimeMillis - j > hj.vhC) {
                    if (service != null) {
                        service.cancel();
                    ...
                    try {
                        ((AlarmManager) context.getSystemService("alarm")).set(0, (hj.vhC + currentTimeMillis) - 2000, PendingIntent.getService(context, 1000001, intent, 134217728));
                    } catch (Throwable e) {
                        RecoveryLog.printErrStackTrace(TAG, e, "alarmManager.setAlarm", new Object[0]);
                    }
                    RecoveryLog.i(TAG, "startAlarm pendingIntent success: %d will report %s", Integer.valueOf(r2.hashCode()), Util.gS(j));
                } else {
    ...
    public static void startAlarmAfter(Context context, String str, long j) {
        if (!Util.oW(str)) {
            ...
            RecoveryLog.i(TAG, "startAlarmAfter pendingIntent:%d %s", Integer.valueOf(service.hashCode()), Util.gS(r2));
        }
    }
