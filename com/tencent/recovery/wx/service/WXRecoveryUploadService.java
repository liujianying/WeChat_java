...
public class WXRecoveryUploadService extends RecoveryUploadService {
    ...
    protected boolean tryToUploadData() {
        ...
        if (a.size() == 0 || uploadData) {
            RecoveryReporter.bH(this, "HandleStatus");
        }
        return uploadData;
    }
    ...
    protected boolean uploadData(List<RecoveryHandleItem> list) {
        ...
        String str = "0x26060736";
        for (RecoveryHandleItem recoveryHandleItem : list) {
            try {
                String str2;
                jSONArray.put(new JSONObject().put("tag", UNIT_REPORT_TAG).put("info", recoveryHandleItem.aAL).put("uin", recoveryHandleItem.fMk).put("deviceUUID", hl).put("time", Util.gS(recoveryHandleItem.timestamp)).put("cver", recoveryHandleItem.clientVersion).put("processName", recoveryHandleItem.processName).put("phoneStatus", recoveryHandleItem.vhx));
    ...
    private boolean pushData(JSONArray jSONArray, String str) {
        try {
            ...
            String toLowerCase = EncryptUtil.u(String.format("weixin#$()%d%d", new Object[]{Integer.decode(str), Integer.valueOf(bytes.length)}).getBytes()).toLowerCase();
            bytes = EncryptUtil.r(bytes);
            ...
            if (!(hq == null || hq.length() == 0)) {
                append.append("&username=").append(hq);
            }
